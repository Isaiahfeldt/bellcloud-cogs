    ...
    async def emote_info(self, interaction: discord.Interaction, name: str):
        try:
            ...
            if result is not None:
                ...
                embed = discord.Embed(color=0x00ff00)
                embed.set_author(name=emote_name, url=f"https://media.bellbot.xyz/emote/{file_path}", icon_url=f"https://media.bellbot.xyz/emote/{file_path}")
                ...
                embed.set_footer(text=f"Emote ID: {emote_id} | {format_time_ago(timestamp)} by {author.display_name}", icon_url=author.display_avatar.url)
