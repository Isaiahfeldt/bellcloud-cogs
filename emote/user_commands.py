...
class UserCommands(commands.Cog):
    ...
    async def handle_apply_effect_user(self, interaction: discord.Interaction, user: discord.User):
        """Context menu command to apply effects to a user's avatar."""
        ...
        try:
                ...
                async with session.get(str(avatar_url)) as resp:
                    if resp.status != 200:
                        await interaction.response.send_message(
                            "I couldn't download the user's avatar image.",
                            ephemeral=True
                        )
                        return
            ...
            await interaction.response.send_message(
                f"An error occurred while downloading the avatar: {str(e)}",
                ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"Select effect(s) to apply to {user.display_name}'s avatar:",
            view=view,
            ephemeral=True
        )
